...
export const AdManager = ({
  ...
  return (
    <section className="grid items-stretch gap-6 xl:grid-cols-2">
      <form
        className="glass-card h-full rounded-[2rem] p-6 lg:p-7"
      ...
      >
        <div className="mb-5 space-y-2">
          ...
          <h2 className="text-2xl font-semibold">
            {draft._id ? "Edit ad campaign" : "Launch a new campaign"}
          </h2>
          <p className="text-sm leading-6 text-slate-500">
            Create sponsored placements that can be inserted into the
            personalized feed.
          </p>
        </div>
        ...
        <div className="grid gap-4">
          ...
          <input
            ...
            onChange={(e) =>
              setDraft((cur) => ({ ...cur, imageUrl: e.target.value }))
            }
            className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"
          ...
          <input
            ...
            onChange={(e) =>
              setDraft((cur) => ({ ...cur, targetLink: e.target.value }))
            }
            className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"
          ...
          />
          <input
            value={draft.ctaLabel ?? "Learn more"}
            onChange={(e) =>
              setDraft((cur) => ({ ...cur, ctaLabel: e.target.value }))
            }
            className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm"
            ...
            {topics.map((topic) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-full px-3 py-2 text-sm font-semibold ${active ? "bg-pine text-white" : "bg-slate-100 text-slate-700"}`}
                >
                  {topic}
                </button>
              );
        ...
        <div className="mt-5 flex flex-wrap gap-3">
          ...
          >
            {draft._id ? "Update ad" : "Create ad"}
          </button>
          {draft._id ? (
            <button
              type="button"
              onClick={() => setDraft(blankAd)}
      ...
      <div className="glass-card h-full rounded-[2rem] p-5 lg:p-6">
        <div className="border-b border-slate-200 pb-4">
          ...
          <p className="mt-1 text-sm text-slate-500">
            Review active creatives and jump back into edit mode quickly.
          </p>
        </div>
