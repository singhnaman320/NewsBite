...
export const api = {
  ...
  login: (body: { email: string; password: string; role?: "admin" | "user" }) =>
    request<AuthResponse & { message?: string }>("/auth/login", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  me: (token: string) => request<{ user: User }>("/auth/me", {}, token),
  categories: (token: string) =>
    request<{ categories: string[] }>("/feed/categories", {}, token),
  feed: (token: string, tab: string, page: number, limit = 8) =>
    request<FeedResponse>(
      `/feed?tab=${encodeURIComponent(tab)}&page=${page}&limit=${limit}`,
      {},
      token,
    ),
  ...
  toggleSaved: (token: string, articleId: string) =>
    request<{ saved: boolean; savedArticles: string[]; message?: string }>(
      `/users/saved/${articleId}`,
      { method: "POST" },
      token,
    ),
  agents: (token: string) =>
    request<{ agents: Agent[] }>("/admin/agents", {}, token),
  saveAgent: (token: string, body: Partial<Agent>) =>
    body._id
      ? request<{ agent: Agent; message?: string }>(
          `/admin/agents/${body._id}`,
          {
            method: "PATCH",
            body: JSON.stringify(body),
          },
  ...
  runAgent: (token: string, id: string) =>
  ...
  ads: (token: string) =>
    request<{ ads: AdCampaign[] }>("/admin/ads", {}, token),
  saveAd: (token: string, body: Partial<AdCampaign>) =>
    body._id
      ? request<{ ad: AdCampaign; message?: string }>(
          `/admin/ads/${body._id}`,
          {
            method: "PATCH",
            body: JSON.stringify(body),
  ...
  analytics: (token: string) =>
    request<{ analytics: AnalyticsRow[] }>("/admin/analytics", {}, token),
  trackView: (token: string, adId: string, articleId?: string) =>
    request<{ tracked: boolean }>(
      "/tracking/view",
      {
        method: "POST",
